...
public class DataBaseHandler_1600x900 {
	    ...
	    try {
	         ...
	         ResultSetMetaData rsmd = rs.getMetaData(); // 获得rs结果集中列属性信息
	             ...
	             for (int i = 1; i <= rsmd.getColumnCount(); ++i)
	               ...
	               TableViewInquiryContract_1600x900(tabDemo);   
	                close(sm,rs);
	      }catch (SQLException e) {
	    ...
	    try {
	         ...
	         ResultSetMetaData rsmd = rs.getMetaData(); // 获得rs结果集中列属性信息
	             for (int i = 1; i <= rsmd.getColumnCount(); ++i)
	               ...
	               while (rs.next()); // 利用循环获得所有记录 
	               tabDemo = new JTable(rows, columnHeads); // 将获得的行列数据信息作为参数重新构造表格视图
	               TableViewOneTouchCredit_1600x900(tabDemo);   
	                close(sm,rs);
	      }catch (SQLException e) {
...
//			    + " and rownum<10000";
     ...
     * @param table 
     */  
	public static void TableViewInquiryContract_1600x900(JTable tabDemo){  
        ...
        databasetools.ui.frame_1600x900.InquiryContract_1600x900.DataDisplayList.getViewport().add(tabDemo); 
     ...
     * @param table 
     */  
	public static void TableViewOneTouchCredit_1600x900(JTable tabDemo){  
        jth = tabDemo.getTableHeader();
        RowSorter<TableModel> sorter = new TableRowSorter<TableModel>(tabDemo.getModel());
        tabDemo.setRowSorter(sorter);
        //设置数据与单元格边框的眉边距
        ...
        fitTableColumns(tabDemo);
        //将JTable加入到带纵向滚动条的面板中
        databasetools.ui.frame_1600x900.OneTouchCredit_1600x900.DataDisplayList.getViewport().add(tabDemo); 
